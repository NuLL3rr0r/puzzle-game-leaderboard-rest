...
#define     TokenXML_URI_TEMPLATE                    L"NowruzPuzzle/Token/XML"
...
struct PublicAPIResource::Impl
{
    ...
    std::unique_ptr<NowruzPuzzle::ServiceContract> ServiceContractPtr;
...
{
    m_pimpl->ServiceContractPtr = std::make_unique<NowruzPuzzle::ServiceContract>();
    m_pimpl->ServiceContractPtr->Register(PlayerRankJSON_URI_TEMPLATE);
    m_pimpl->ServiceContractPtr->Register(PlayerRankXML_URI_TEMPLATE);
...
void PublicAPIResource::handleRequest(const Wt::Http::Request &request, Wt::Http::Response &response)
{
            ...
            if (boost::algorithm::contains(uriTemplate, L"/{TOKEN}")) {
                if (!m_pimpl->IsValidToken(args[args.size() - 1])) {
                    if (boost::algorithm::contains(uriTemplate, L"/JSON")) {
                        throw NowruzPuzzle::JSONException(INVALID_TOKEN_ERROR);
                    } else if (boost::algorithm::contains(uriTemplate, L"/XML")) {
                        throw NowruzPuzzle::XMLException(INVALID_TOKEN_ERROR);
                    } else {
                        throw NowruzPuzzle::Exception(INVALID_TOKEN_ERROR);
                    }
                }
            }

            if (uriTemplate == PlayerRankJSON_URI_TEMPLATE) {

                m_pimpl->PlayerRankJSON(args[0], outResponse);
                PrintJSON(response, outResponse);

            } else if (uriTemplate == PlayerRankXML_URI_TEMPLATE) {

                m_pimpl->PlayerRankXML(args[0], outResponse);
    ...
    catch (const NowruzPuzzle::JSONException &ex) {
        PrintJSON(response, ex.What());
    }

    catch (const NowruzPuzzle::XMLException &ex) {
        PrintXML(response, ex.What());
    }

    catch (const NowruzPuzzle::Exception &ex) {
                                               ...
                                               boost::property_tree::wptree &out_tree)
{
    ...
    out_tree.put(L"NowruzPuzzle.version.major", "1");
    out_tree.put(L"NowruzPuzzle.version.minor", "0");
    out_tree.put(L"NowruzPuzzle.version.compat.major", "1");
    out_tree.put(L"NowruzPuzzle.version.compat.minor", "0");
    ...
    out_tree.add_child(L"NowruzPuzzle.titles", titlesTree);
    ...
    out_tree.add_child(L"NowruzPuzzle.data", dataTree);
}
